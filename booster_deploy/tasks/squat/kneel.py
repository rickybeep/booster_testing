...
def _csv(metadata: dict[str, str], key: str) -> list[str]:
    try:
        ...
        raise ValueError(f"Kneel ONNX metadata is missing '{key}'") from exc
...
class KneelPolicy(Policy):
    """Stateful deployment wrapper for the kneel ONNX.

    Ported from the `sitdown-testing` branch. The model carries its own
    trajectory state machine and returns the next reference frame on every
    call; deployment only feeds the returned state back in.
    """

    def __init__(self, cfg: KneelPolicyCfg, controller: BaseController):
        super().__init__(cfg, controller)
        self.cfg = cfg
        self.robot = controller.robot
        model_path = Path(self.task_path, cfg.checkpoint_path)
        if not model_path.is_file():
            raise FileNotFoundError(f"Kneel policy not found: {model_path}")
    ...
    def _validate_model(self) -> None:
        ...
        if tuple(inputs) != INPUT_NAMES:
            raise ValueError(
                f"Unexpected kneel ONNX inputs: {tuple(inputs)}; expected {INPUT_NAMES}"
            )
        if tuple(outputs) != OUTPUT_NAMES:
            raise ValueError(
                f"Unexpected kneel ONNX outputs: {tuple(outputs)}; expected {OUTPUT_NAMES}"
            )
        if inputs["obs"].shape != [1, OBSERVATION_SIZE]:
            raise ValueError(
                "Kneel ONNX obs must be "
                f"[1, {OBSERVATION_SIZE}], got {inputs['obs'].shape}"
            )
        if inputs["squat_state_in"].shape != [1, 3]:
            raise ValueError("Kneel ONNX state must be [1, 3]")
        observations = tuple(_csv(self.metadata, "observation_names"))
        if observations != EXPECTED_OBSERVATIONS:
            raise ValueError(
                f"Unexpected kneel observation layout: {observations}"
            )
        if len(_csv(self.metadata, "joint_names")) != self.robot.num_joints:
            raise ValueError("Kneel ONNX joint count does not match the robot")
    ...
    def _validate_robot_config(self) -> None:
        ...
        if resolved_names != self.robot.cfg.joint_names:
            raise ValueError("Kneel ONNX joint order does not match the K1 config")
        if not np.allclose(
        ...
        if self.action_scale.shape != (len(self.action_joint_names),):
            raise ValueError(
                "Kneel ONNX action scale must match the 20 non-head joints"
            )
    ...
    def compute_observation(self) -> np.ndarray:
        ...
        if observation.shape != (OBSERVATION_SIZE,):
            raise RuntimeError(f"Built invalid kneel observation {observation.shape}")
    ...
    def is_standing_pose(self) -> bool:
        """Workflow hook shared with `SquatPolicy`: standing means the
        embedded trajectory has returned to its standing sentinel."""
        return self.is_standing_reference()


@configclass
class KneelPolicyCfg(PolicyCfg):
    constructor = KneelPolicy
    checkpoint_path: str = MISSING
    # The kneel policy runs on the gains embedded in its ONNX metadata only.
    gain_overrides_path: str | None = None
