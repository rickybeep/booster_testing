...
from tasks.squat.squat import JOINT_ALIASES
...
STANDING_STATE = np.asarray([[0, 0, 1]], dtype=np.int64)
...
def _csv(metadata: dict[str, str], key: str) -> list[str]:
    try:
        ...
        raise ValueError(f"Sit ONNX metadata is missing '{key}'") from exc
...
class SitPolicy(Policy):
    """Stateful deployment wrapper for the sit motion-tracking ONNX.

    Ported from the kneel policy on the `kneeling` branch. The model carries
    its own trajectory state machine and returns the next reference frame on
    every call; deployment only feeds the returned state back in.
    """

    def __init__(self, cfg: SitPolicyCfg, controller: BaseController):
        super().__init__(cfg, controller)
        self.cfg = cfg
        self.robot = controller.robot
        model_path = Path(self.task_path, cfg.checkpoint_path)
        if not model_path.is_file():
            raise FileNotFoundError(f"Sit policy not found: {model_path}")
        ...
        # The policy artifact owns its deployment gains; they are installed on
        # the shared robot only when this policy is activated.
    ...
    def _validate_model(self) -> None:
        ...
        if tuple(inputs) != INPUT_NAMES:
            raise ValueError(
                f"Unexpected sit ONNX inputs: {tuple(inputs)}; expected {INPUT_NAMES}"
            )
        if tuple(outputs) != OUTPUT_NAMES:
            raise ValueError(
                f"Unexpected sit ONNX outputs: {tuple(outputs)}; expected {OUTPUT_NAMES}"
            )
        if inputs["obs"].shape != [1, OBSERVATION_SIZE]:
            raise ValueError(
                "Sit ONNX obs must be "
                f"[1, {OBSERVATION_SIZE}], got {inputs['obs'].shape}"
            )
        if inputs["squat_state_in"].shape != [1, 3]:
            raise ValueError("Sit ONNX state must be [1, 3]")
        observations = tuple(_csv(self.metadata, "observation_names"))
        if observations != EXPECTED_OBSERVATIONS:
            raise ValueError(f"Unexpected sit observation layout: {observations}")
        if len(_csv(self.metadata, "joint_names")) != self.robot.num_joints:
            raise ValueError("Sit ONNX joint count does not match the robot")
        for key in ("joint_stiffness", "joint_damping"):
            if _float_csv(self.metadata, key).shape != (self.robot.num_joints,):
                raise ValueError(f"Sit ONNX {key} must match the robot joints")
    ...
    def _validate_robot_config(self) -> None:
        ...
        if resolved_names != self.robot.cfg.joint_names:
            raise ValueError("Sit ONNX joint order does not match the K1 config")
        if not np.allclose(
            ...
            raise ValueError("K1 default_joint_pos does not match sit ONNX metadata")
        if self.action_scale.shape != (len(self.action_joint_names),):
            raise ValueError("Sit ONNX action scale must match the 20 non-head joints")

    def reset(self) -> None:
        self.sit_state = STANDING_STATE.copy()
        ...
        seeded = self.session.run(
            list(OUTPUT_NAMES),
            {
                ...
                "squat_state_in": self.sit_state,
            },
        )
        self.sit_state = seeded[1]
    ...
    def compute_observation(self) -> np.ndarray:
        ...
        reference_quat = torch.from_numpy(self.ref_body_quat_w[self.anchor_index])
        ...
        if observation.shape != (OBSERVATION_SIZE,):
            raise RuntimeError(f"Built invalid sit observation {observation.shape}")
    ...
    def is_standing_pose(self) -> bool:
        """Standing means the embedded trajectory is back at its sentinel."""
        return bool(np.array_equal(self.sit_state, STANDING_STATE))


@configclass
class SitPolicyCfg(PolicyCfg):
    constructor = SitPolicy
